...
document.querySelector('#btnThem').onclick = function () {
    ...
    valid = kiemTra.kiemTraRong(nv.taiKhoan, 'tbTKNV', 'Tài khoản') &
        kiemTra.kiemTraRong(nv.hoTen, 'tbTen', 'Họ tên') &
        kiemTra.kiemTraRong(nv.email, 'tbEmail', 'Email') &
        kiemTra.kiemTraRong(nv.matKhau, 'tbMatKhau', 'Mật khẩu') &
        kiemTra.kiemTraRong(nv.ngayLam, 'tbNgay', 'Ngày làm') &
        kiemTra.kiemTraRong(nv.luongCoBan, 'tbLuongCB', 'Lương cơ bản') &
        kiemTra.kiemTraRong(nv.chucVu, 'tbChucVu', 'Chức Vụ') &
        kiemTra.kiemTraRong(nv.gioLam, 'tbGioLam', 'Giờ làm');

    //Kiểm tra tài khoản 4-6 ký số
    valid = valid & kiemTra.kiemTraKySo(nv.taiKhoan, 'tbTKNV', 'Tài khoản', 4, 6);
    // Kiểm tra số taiKhoan khi thêm mới không được trùng với tài khoản đã có trong table
    valid = valid & kiemTra.kiemTraTonTai(nv.taiKhoan, mangNhanVien, 'taiKhoan', 'tnTKNV', 'Tài khoản');
    // Kiểm tra tên nhân viên (phải là chữ, không để trống)
    valid = valid & kiemTra.kiemTraKyTu(nv.hoTen, 'tbTen', 'Họ tên');
    //Kiểm tra email - đúng định dạng
    valid = valid & kiemTra.kiemTraEmail(nv.email, 'tbEmail', 'Email');
    // Kiểm tra mật khẩu (4-6 ký tự, chứa ít nhất 1 số, 1 in hoa, 1 đặc biệt và không trống)
    valid = valid & kiemTra.kiemTraMatKhau(nv.matKhau, 'tbMatKhau', 6, 10);
    // kiểm tra ngày làm ( không để trống, định dạng mm/dd/yyyy)
    valid = valid & kiemTra.kiemTraNgayThang(nv.ngayLam, 'tbNgay');
    // Kiểm tra lương cơ bản (1 triệu đến 20 triệu, không để trống)
    valid = valid & kiemTra.kiemTraGiaTri(Number(nv.luongCoBan), 'tbLuongCB', 'lương cơ bản', 1e+6, 20e+6);
    //Kiểm tra chức vụ chọn phải hợp lệ    
    // Kiểm tra số giờ làm (80-200, không để trống)
    valid = valid & kiemTra.kiemTraGiaTri(nv.gioLam, 'tbGioLam', 'Giờ làm', 80, 200);
    if (!valid) {
        return;
    };
