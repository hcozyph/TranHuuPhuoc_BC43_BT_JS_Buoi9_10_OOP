...
function renderTable(array) {
    ...
    for (var index = 0; index < array.length; index++) {
        var renderObject = array[index];
        htmlString += `
            <tr>
                <td>${renderObject.taiKhoan}</td>
                <td>${renderObject.hoTen}</td>
                <td>${renderObject.email}</td>
                <td>${renderObject.ngayLam}</td>
                <td>${renderObject.chucVu}</td>
                <td>${renderObject.tongLuong.toLocaleString()}</td>
                <td>${renderObject.xepLoai}</td>
                <td>
                    <button onclick = "layThongTin('${renderObject.taiKhoan}')" class = 'btn btn-warning' data-bs-toggle="modal" data-bs-target="#exampleModal" ><i class="fa fa-wrench"></i></button>
                </td>            
                <td>
                    <button class = 'btn btn-danger' onclick = "xoaNhanVien('${renderObject.taiKhoan}')"><i class="fa fa-times"></i></button>
                </td>
